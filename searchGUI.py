...
def searchAnime():
    ...
    if anime_title != '':
        ...
        #if 'scrollbar' not in locals():
        #    scrollbar = tk.Scrollbar(master=app)
        #    scrollbar.pack( side = tk.RIGHT, fill = 'y')

        animesList = ctk.CTkFrame(master=cvs, bg_color='red')
        #animesList['yscrollcommand'] = scrollbar.set
        animesList.pack(fill=tk.BOTH, expand=True)
        #if 'scrollbar' in locals():
            #scrollbar.config( command = animesList.yview )
        if 'anime_title_old' not in locals():
            anime_title_old = ''
...
cvs =tk.Canvas(master=main_frame)
cvs.pack(expand=True,side=tk.LEFT,fill=tk.BOTH)

scrollbar = tk.Scrollbar(master=main_frame, orient=tk.VERTICAL, command=cvs.yview)
scrollbar.pack( side = tk.RIGHT, fill = 'y')

cvs.config(yscrollcommand=scrollbar.set)
cvs.bind('<Configure>',lambda e:cvs.configure(scrollregion=cvs.bbox('all')))

searchFm = ctk.CTkFrame(master=cvs, height=28)
searchFm.place(relx=0.5, rely=0.5, anchor=tk.CENTER)

cvs.create_window(0, 0, window=searchFm, anchor="nw")
def update_size(e=None):
    cvs["scrollregion"] = cvs.bbox("all")
cvs.bind('<Configure>', update_size)
cvs.after_idle(update_size)
